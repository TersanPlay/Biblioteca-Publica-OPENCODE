...
export function ReturnsPage() {
  ...
  const confirmReturn = async () => {
    ...
    try {
      ...
      toast.success('Devolução registrada', `${returning.bookTitleSnapshot} devolvido`);
      setReturning(null);
      setReturnCondition('');
  ...
  return (
    <div className="space-y-5">
                  ...
                  {data.items.map((l) => {
                    ...
                    return (
                      <TR key={l.$id} className={overdue ? 'bg-destructive-soft/30' : undefined}>
                        ...
                        <TD className="max-w-44 truncate font-semibold text-ink">{l.readerNameSnapshot}</TD>
                        <TD className="max-w-60">
                          <span className="block line-clamp-1 font-medium text-ink/85">{l.bookTitleSnapshot}</span>
                        </TD>
      ...
      <ConfirmDialog
        ...
        description={
          returning
            ? `${returning.bookTitleSnapshot} — leitor ${returning.readerNameSnapshot}.`
            : undefined
        }
